...
namespace SampleClassification.ConsoleApp
{
    ...
    {
        public const string RetrainFilePath =  @"/Users/imandanial/Desktop/Project_PdM_Dashboard/sensor_data.csv";
        ...
        public static IEstimator<ITransformer> BuildPipeline(MLContext mlContext)
        {
            ...
            var pipeline = mlContext.Transforms.Categorical.OneHotEncoding(@"MachineID", @"MachineID", outputKind: OneHotEncodingEstimator.OutputKind.Indicator)      
                                    .Append(mlContext.Transforms.ReplaceMissingValues(new []{new InputOutputColumnPair(@"Temperature", @"Temperature"),new InputOutputColumnPair(@"Vibration", @"Vibration"),new InputOutputColumnPair(@"HoursRun", @"HoursRun"),new InputOutputColumnPair(@"Pressure", @"Pressure")}))      
                                    .Append(mlContext.Transforms.Concatenate(@"Features", new []{@"MachineID",@"Temperature",@"Vibration",@"HoursRun",@"Pressure"}))      
                                    .Append(mlContext.Transforms.Conversion.MapValueToKey(outputColumnName:@"DidItFail",inputColumnName:@"DidItFail",addKeyValueAnnotationsAsText:false))      
                                    .Append(mlContext.MulticlassClassification.Trainers.OneVersusAll(binaryEstimator:mlContext.BinaryClassification.Trainers.FastTree(new FastTreeBinaryTrainer.Options(){NumberOfLeaves=4,MinimumExampleCountPerLeaf=20,NumberOfTrees=4,MaximumBinCountPerFeature=254,FeatureFraction=1,LearningRate=0.09999999999999998,LabelColumnName=@"DidItFail",FeatureColumnName=@"Features",DiskTranspose=false}),labelColumnName: @"DidItFail"))      
